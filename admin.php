...
if ($id == "login" || substr($url, -1) == "p") {
...
} elseif (!isset($_SESSION["ssmonline"])) {
  ...
  if ($API->connect($iphost, $userhost, decrypt($passwdhost))){
    ...
    if ($currency == in_array($currency, $cekindo['indo'])) {
      echo "<script>alert('SSM-ONLINE not connected!".$nl."Please check the IP, User, Password and port API must be enabled.".$nl."If using a VPN connection, make sure the VPN is connected.')</script>";
    }else{
      echo "<script>alert('SSM-ONLINE not connected!".$nl."Please check the IP, User, Password and port API must be enabled.')</script>";
    }
    if($c == "settings"){
...
?>
<script src="js/ssmonline-ui.<?= $theme; ?>.min.js"></script>
<script src="js/ssmonline.js?t=<?= str_replace(" ","_",date("Y-m-d H:i:s")); ?>"></script>
