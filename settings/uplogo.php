...
// Allow certain file formats
    if (basename($_FILES["UploadLogo"]["name"] != "logo-" . $session . ".png")) {
      if ($currency == in_array($currency, $cekindo['indo'])) {
        $galat = '<div class="box bg-danger"></i> Alert!<br>  Can only upload logos-' . $session . '.png. </div>';
      } else {
...
// if everything is ok, try to upload file
      ...
      if (move_uploaded_file($_FILES["UploadLogo"]["tmp_name"], $logo_file)) {
        if ($currency == in_array($currency, $cekindo['indo'])) {
          $galat = '<div class="box bg-success"></i> Alert!<br>  Success!</h5> File ' . basename($_FILES["UploadLogo"]["name"]) . ' has been uploaded. </div>';
        } else {
